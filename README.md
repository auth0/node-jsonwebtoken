...
const privateKey = "shhhhh"; // Your secret key
// const privateKey = process.env.PRIVATE_KEY   ; // Your secret key from .env

// verify a token symmetric - synchronous
const decodedSync = jwt.verify(token, privateKey);
...
try {
  const decodedInvalidSync = jwt.verify(token, "wrong-secret");
} catch (err) {
  // err
}
